	...
	public ResponseEntity<?> list(HttpEntity<String> _httpEntity) {
		...
		if (Flag.flag) {
			try {
				...
				accessToken = AccessToken.getAccessToken("/list");
		...
		if (Flag.flag) {
			try {
				...
				UriComponents builder = UriComponentsBuilder.fromHttpUrl(this.lightnetUrl + GET_LIST_HTTPS_URL)
						.queryParams(reqMap)
						.build(true);
	...
	public ResponseEntity<?> list1(HttpEntity<String> _httpEntity) {
		...
		if (Flag.flag) {
			try {
				...
				UriComponents builder = UriComponentsBuilder.fromHttpUrl(this.lightnetUrl + GET_LIST1_HTTPS_URL)
						.queryParams(reqMap)
						.build(true);
