...
public class List11Process {
	...
	public LnsStream process(LnsStream reqLnsStream) throws Exception {
		...
		if (Flag.flag) {
			...
			resLnsStream = new LnsStream(String.format("%04d%7.7s%s", resStream.length() + 7, "0210701", resStream));
		}
		...
		if (Flag.flag) {
			try {
				...
				ResponseEntity<String> response = RestTemplateConfig.get(RestTemplateType.SETENV).exchange(
						this.projEnvUrlProperties.getMapper() + "/mapper/list11/req/s2j"
						, HttpMethod.POST
						, reqHttpEntity
						, String.class);
	...
	public LnsJson httpLinkPost(LnsJson lnsJson) throws Exception {
		...
		if (Flag.flag) {
			try {
				...
				ResponseEntity<String> response = RestTemplateConfig.get(RestTemplateType.SETENV).exchange(
						this.projEnvUrlProperties.getLink() + "/link/list11/get"
						, HttpMethod.POST
						, reqHttpEntity
						, String.class);
	...
	public LnsJson mapperResJsonToStr(LnsJson lnsJson) throws Exception {
		...
		if (Flag.flag) {
			try {
				...
				ResponseEntity<String> response = RestTemplateConfig.get(RestTemplateType.SETENV).exchange(
						this.projEnvUrlProperties.getMapper() + "/mapper/list11/res/j2s"
						, HttpMethod.POST
						, reqHttpEntity
						, String.class);
