...
public class ApisRestController {
	...
	public ResponseEntity<?> getCalculation(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			...
			log.info("LNS01 >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("LNS01 >>>>> Body = {}", strBody);
		}
		...
		if (Flag.flag) {
			...
			if (Flag.flag) {
				...
				log.info("LNS01 >>>>> reqLnsStream = {}", JsonPrint.getInstance().toPrettyJson(reqLnsStream));
			}
			...
			if (Flag.flag) {
				...
				log.info("LNS01 >>>>> resLnsStream = {}", JsonPrint.getInstance().toPrettyJson(resLnsStream));
			}
			
			((ObjectNode) jsonNode).put("resStream", resLnsStream.getData());
			log.info("LNS01  >>>>> jsonNode = {}", jsonNode.toPrettyString());
		}
