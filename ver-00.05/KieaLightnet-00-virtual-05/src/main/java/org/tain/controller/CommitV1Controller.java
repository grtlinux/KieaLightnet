...
@RestController
@RequestMapping(value = {"/v1/remittances.commit"})
@Slf4j
public class CommitV1Controller {
	...
	public ResponseEntity<?> commit(HttpEntity<String> httpEntity) throws Exception {
		...
		if (Flag.flag) {
			map = new ObjectMapper().readValue(
					new File(System.getenv("HOME") + this.lnsEnvVirtualProperties.getCommitFile())
