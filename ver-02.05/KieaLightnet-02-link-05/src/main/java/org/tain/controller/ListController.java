...
public class ListController {
	...
	///////////////////////////////////////////////////////////////////////////
	///////////////////////////////////////////////////////////////////////////
	// http://localhost:18082/v0.3/link/list

	private String GET_LIST_HTTPS_URL = "/v1/remittances";
	
	@CrossOrigin(origins = {"/**"})
	@RequestMapping(value = {"/list"}, method = {RequestMethod.GET, RequestMethod.POST})
	public ResponseEntity<?> list(HttpEntity<String> _httpEntity) {
		...
		if (Flag.flag) {
			try {
				...
				accessToken = this.accessToken.getAccessToken("/list");
		...
		if (Flag.flag) {
			try {
				...
				UriComponents builder = UriComponentsBuilder.fromHttpUrl(this.lnsEnvLinkProperties.getLightnetUrl() + GET_LIST_HTTPS_URL)
						.queryParams(reqMap)
						.build(true);
	...
	public ResponseEntity<?> list1(HttpEntity<String> _httpEntity) {
		...
		if (Flag.flag) {
			try {
				...
				UriComponents builder = UriComponentsBuilder.fromHttpUrl(this.lnsEnvLinkProperties.getLightnetUrl() + GET_LIST1_HTTPS_URL)
						.queryParams(reqMap)
						.build(true);
				response = RestTemplateConfig.get(RestTemplateType.SSL01).exchange(
						builder.toString()
